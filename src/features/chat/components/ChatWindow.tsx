...
import ChatGroupSettingsUI from './ChatGroupSettingsUI';

interface ChatWindowProps {
  conversation?: Conversation | null;
}

type ChatWindowView = 'messages' | 'group-settings' | 'contact-info';
...
export default function ChatWindow({ conversation }: ChatWindowProps) {
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-zinc-900">
        <div className="text-center text-zinc-500">
          ...
          <p className="text-sm">
            Escolha uma conversa na lista para começar a mensagens
          </p>
        </div>
      </div>
    );
  ...
  const handleGroupSettings = () => {
    if (conversation.isGroup) {
      setChatWindowView('group-settings');
    } else {
      setChatWindowView('contact-info');
    }
  };
    ...
    <div className="flex-1 flex flex-col bg-zinc-900">
      {chatWindowView === 'messages' && (
        <>
                ...
                <p className="text-xs text-zinc-400 truncate">
                  {conversation.isGroup
                    ? conversation.membersId &&
                      conversation.membersId.length > 0
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="flex items-end gap-2">
                            ...
                            <span className="text-xs text-zinc-300 shrink-0 self-end">
          ...
          <footer className="h-16 bg-zinc-800 border-t border-zinc-700 flex items-center gap-2 px-4 shrink-0">
            <input
              type="text"
              value={messageInput}
